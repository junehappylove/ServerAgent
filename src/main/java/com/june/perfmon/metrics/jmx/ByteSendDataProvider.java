 ...
 */
class ByteSendDataProvider extends AbstractJMXDataProvider {
	MBeanServerConnection mBeanServerConn;

	public ByteSendDataProvider(MBeanServerConnection mBeanServerConn, boolean diff) throws Exception {
		super(mBeanServerConn, diff);
		this.mBeanServerConn = mBeanServerConn;
	}

	protected String getMXBeanType() {
		return JMXDataConst.JMX_BYTE_SEND;
	}
	...
	protected long getValueFromBean(Object bean) {
		...
		try {
			...
			for (ObjectName obj : smbi) {
				...
				long now = (long) mBeanServerConn.getAttribute(objname, "bytesSent");
				result = result + now;
			}
